...
public class PriceTierController {
    ...
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getAllPriceTiers() {
        ...
        return ResponseEntity
                .ok(new ResponseWrapper<>(HttpStatus.OK.value(), "Price tiers retrieved successfully", priceTiers));
    }
    ...
    public ResponseEntity<ResponseWrapper<PriceTier>> getPriceTierById(@PathVariable Long id) {
        ...
        return priceTier
                .map(tier -> ResponseEntity
                        .ok(new ResponseWrapper<>(HttpStatus.OK.value(), "Price tier retrieved successfully", tier)))
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(new ResponseWrapper<>(HttpStatus.NOT_FOUND.value(), "Price tier not found", null)));
    }
    ...
    public ResponseEntity<ResponseWrapper<PriceTier>> createPriceTier(@RequestBody PriceTier priceTier) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(
                new ResponseWrapper<>(HttpStatus.CREATED.value(), "Price tier created successfully", createdPriceTier));
    }
    ...
    public ResponseEntity<ResponseWrapper<PriceTier>> updatePriceTier(@PathVariable Long id,
        ...
        try {
            ...
            return ResponseEntity.ok(
                    new ResponseWrapper<>(HttpStatus.OK.value(), "Price tier updated successfully", updatedPriceTier));
    ...
    public ResponseEntity<ResponseWrapper<Void>> deletePriceTier(@PathVariable Long id) throws ProductException {
        try {
            ...
            return ResponseEntity
                    .ok(new ResponseWrapper<>(HttpStatus.OK.value(), "Price tier deleted successfully", null));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ResponseWrapper<>(HttpStatus.NOT_FOUND.value(), e.getMessage(), null));
        }
    ...
    public ResponseEntity<ResponseWrapper<Optional<PriceTier>>> getPriceTierByTierId(@PathVariable Long tierId) {
        ...
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tier retrieved successfully by tier ID", priceTier));
    }
    ...
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByProduct(@RequestBody Products product) {
        ...
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by product", priceTiers));
    }
    ...
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByProductId(@PathVariable Long productId) {
        ...
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by product ID", priceTiers));
    }

    @GetMapping("/by-min-quantity/{minimumQuantity}")
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByMinimumQuantity(
            @PathVariable Integer minimumQuantity) {
        List<PriceTier> priceTiers = priceTierService.findPriceTiersByMinimumQuantity(minimumQuantity);
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by minimum quantity", priceTiers));
    }

    @GetMapping("/by-price/{pricePerUnit}")
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByPricePerUnit(
            @PathVariable BigDecimal pricePerUnit) {
        List<PriceTier> priceTiers = priceTierService.findPriceTiersByPricePerUnit(pricePerUnit);
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by price per unit", priceTiers));
    }

    @GetMapping("/by-max-quantity/{maxQuantity}")
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByMaxQuantity(
            @PathVariable Integer maxQuantity) {
        List<PriceTier> priceTiers = priceTierService.findPriceTiersByMaxQuantity(maxQuantity);
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by maximum quantity", priceTiers));
    }
    ...
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByIsActive(@PathVariable Boolean isActive) {
        ...
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by active status", priceTiers));
    }

    @GetMapping("/created-between")
    public ResponseEntity<ResponseWrapper<List<PriceTier>>> getPriceTiersByCreatedAtBetween(
            @RequestBody LocalDateTime[] dates) {
        if (dates == null || dates.length != 2) {
            return ResponseEntity.badRequest().body(
                    new ResponseWrapper<>(HttpStatus.BAD_REQUEST.value(), "Please provide a start and end date", null));
        }
        ...
        return ResponseEntity.ok(new ResponseWrapper<>(HttpStatus.OK.value(),
                "Price tiers retrieved successfully by creation date range", priceTiers));
    }
}
